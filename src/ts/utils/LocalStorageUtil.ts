...
export default class LocalStorageUtil {
    ...
    public static getUserInfo(): UserInfo {
        let info: any;
        if (Util.isMobile()){
            info = localStorage.getItem(LocalStorageUtil.STORAGES_USER_INFO);
    ...
    public static getUserToken(): UserToken {
        let token: any ;
        if (Util.isMobile()){
           token= localStorage.getItem(LocalStorageUtil.STORAGES_TOKEN);
    ...
    public static removeUserToken(){
        if(localStorage.getItem(LocalStorageUtil.STORAGES_TOKEN)){
            localStorage.removeItem(LocalStorageUtil.STORAGES_TOKEN)
        }
        if(LocalStorageUtil.getCookie(LocalStorageUtil.STORAGES_TOKEN)){
    ...
    /**
     * 保存第三方绑定来源页面,0用户中心  1登录页
     */
    public static addthreeBindSource(pageIndex: string) {
        if (pageIndex == '') throw new Error('保存的第三方绑定来源页面错误!');
        LocalStorageUtil.setCookie(LocalStorageUtil.STORAGES_THIRDBIND_URL_TYPE, pageIndex, -1, true);
    }

    /**
     * 获取第三方绑定来源页面,0用户中心  1登录页
     */
    public static getthreeBindSource(): string {
        const src_channel = LocalStorageUtil.getCookie(LocalStorageUtil.STORAGES_THIRDBIND_URL_TYPE);
