...
export default class Util {
     ...
     * 时间戳转换
     * @param inputTime
     * 根据传入的数字的时间戳的参数转化为 "2018-09-21 00:00:00" 时间格式
     */
     ...
     * 时间格式兼容ios
     */
    public static formateTime(old_time: string) {
        if (old_time == null || old_time == undefined) {
            old_time = '';
        }
        let new_time = old_time.replace(/-/g, '/');
        return new_time;
    }

    /**
     * 判断是否是手机端,是手机端则跳转到手机端站点
     */
    public static checkIsMobile() {
        //判断是否跳转移动站点
        const href = window.location.href;
        const search = window.location.search;
        const arr = href.split("/");
        const pageName = arr[arr.length - 1].split('?')[0];
        const mUrl = GlobalConfig.getMobWebBaseUrl() + '/activitys/' + pageName + search;
     ...
     * @param dict 字符串字典
     */
    public static randomCode(len: number, dict: string) {
    ...
    public static randomPhoneNumber() {
        ...
        return [1, Util.randomCode(1, '345678'), Util.randomCode(9, '0123456789')].join('');
    };

    /**
     * 获取中间四位为*的手机号
     */
    public static getPhoneNum() {
        let num = Util.randomPhoneNumber();
        let reg = /^1([38][0-9]|4[579]|5[0-3,5-9]|6[6]|7[0135678]|9[89])\d{8}$/;
        if (reg.test(num)) {
            return num.slice(0, 3) + '****' + num.slice(7, 11)
        } else {
            return Util.getPhoneNum();
        }
    }
     ...
     * @constructor
     */
    public static ShowToast(msg, timer = 2000) {
        let box = document.createElement("div");
        box.className = "leigod_toastBox";
        document.body.appendChild(box);
        let showdiv = document.getElementsByClassName('leigod_toastBox');
        let html = `<a class="leigod_toast">${msg}</a>`;
        for (let i = 0; i < showdiv.length; i++) {
            showdiv[i].innerHTML = html;
        }
        setTimeout(() => {
            let box = document.getElementsByClassName('leigod_toastBox');
